...
上述 Step 2/3 查询只返回提交状态、总分和编译/运行摘要；逐测试点结果仅由
Step 5 日志接口返回，避免绕过题目日志可见性策略与访问审计。
...
访问审计只记录对 `GET /api/submissions/{submission_id}/log` 的请求；查看提交列表或总分
不会生成审计记录，历史上未经过日志接口的操作也不会回填。前端审计页默认展示全部记录，
并支持按“用户名 + 用户 ID”及题目筛选。
...
生成过程会要求模型给出完整题目、样例/测试点预期输出和逐测试点覆盖目的。后端严格校验 JSON
结构、测试点独立性、规模层次和异常重复输入；样例与测试点输出由模型提供，不会通过标准解自动重算。
