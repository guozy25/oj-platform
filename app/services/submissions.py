...
class SubmissionService:
    ...
    async def create(self, submission: SubmissionCreate, user_id: str) -> dict[str, str]:
        ...
        async with self.database.connection() as connection:
            # Serialize count-and-insert across processes. Each concurrent request
            # sees every submission that already claimed one of the three slots.
            ...
            try:
                ...
                cursor = await connection.execute(
                    "SELECT name FROM languages WHERE name = ? COLLATE NOCASE",
                    (submission.language,),
                )
